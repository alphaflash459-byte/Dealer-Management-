...
const target = `  const handleStockInAIScan = (e: React.ChangeEvent<HTMLInputElement>) => {
...
const replacement = `  const handleStockInImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
...
if (content.includes(target)) {
  ...
  console.log("Target not found logic");
}
