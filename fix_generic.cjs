...
const regex = /else if \(exportDocType === 'stock_count'\) \{[\s\S]*?return \[\s*idx \+ 1,\s*p\.name,\s*verifyOpeningStock \|\| null,\s*rangeStockIn \|\| null,\s*stockReturnPreviousDay \|\| null,\s*rangeStockOut \|\| null,\s*verifyClosingStock \|\| null,\s*[^\]]*\s*\];\s*\}\);\s*\}/g;

code = code.replace(regex, (match) => {
  return `else if (exportDocType === 'stock_count') {
        title = 'របាយការណ៍ស្តុករាប់បញ្ជាក់';
        headers = ["ល.រ", "មុខទំនិញ", "ស្តុកក្នុងឃ្លាំង", "ស្តុកចូល", "ស្តុកលើឡាន", "ស្តុកឡើងឡាន", "ស្តុកសល់", "ស្តុករាប់", "បញ្ជាក់"];
        ...
        rows = products.map((p, idx) => {
          ...
          const verifyDiff = (verifyClosingStock || 0) - (rangeStockCount || 0);
