...
// Replace the buggy handleGeneralExport with a fixed version
const startIdx = code.indexOf('const handleGeneralExport = async () => {');
const endIdx = code.indexOf('const handleExportSelectedUserStockExcel = async () => {');

const fixedExportLogic = `const handleGeneralExport = async () => {
...
code = code.slice(0, startIdx) + fixedExportLogic + code.slice(endIdx);
fs.writeFileSync('src/components/AdminDashboard.tsx', code);
console.log('Fixed export logic typescript errors');
