...
// The broken snippet starts exactly at line 1754 and ends at 1787.
// We'll replace it using regex.
...
const emptyStateRegex = /\{\/\* Invoice Lines list \*\/\}\s*<div className="space-y-1\.5">\s*\{items\.length === 0 \? \([\s\S]*?\) : \(/;
...
// Be careful to only replace if items.length === 0 ? (...) : ( is found
if (emptyStateRegex.test(code)) {
  // Wait, the regex captures up to `) : (`.
  // Let's replace the whole empty state.
  code = code.replace(
    /\{\/\* Invoice Lines list \*\/\}\s*<div className="space-y-1\.5">\s*\{items\.length === 0 \? \(\s*<div className="border border-dashed border-slate-200 rounded-2xl py-8 px-4 text-center text-xs text-slate-400 font-bold bg-slate-50\/30">\s*📦 សូមជ្រើសរើសទំនិញខាងលើ ដើម្បីបន្ថែមចូលក្នុងបញ្ជី\s*<\/div>\s*\) : \(\s*/,
    newEmptyState + '\n                      '
  );
