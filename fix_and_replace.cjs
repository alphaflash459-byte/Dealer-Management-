...
// 1. Restore the bad snippet at 5403
const badSnippet2 = `                      onClick={() => handleAdminបញ្ជាក់Delivery(selectedOrderDetail)}
                                        return (
                        <div key={idx} className="p-2 sm:p-3 bg-slate-50/50 hover:bg-slate-50 transition space-y-1.5">`;

const restoreTarget = `                      onClick={() => handleAdminបញ្ជាក់Delivery(selectedOrderDetail)}
...
const startIndex = content.indexOf(`                      onClick={() => handleAdminបញ្ជាក់Delivery(selectedOrderDetail)}`);
const endIndex = content.indexOf(`                          {/* Product Name */}`, startIndex) + `                          {/* Product Name */}`.length;

if (startIndex !== -1 && endIndex !== -1) {
  content = content.substring(0, startIndex) + restoreTarget + content.substring(endIndex);
  console.log("Restored AdminDashboard");
} else {
  console.log("Could not find restore points");
}

// 2. Now properly replace the editingFullInvoice.items.map block
const targetToReplace = `                      return (
...
if (content.includes(targetToReplace)) {
  ...
  console.log("Successfully replaced editingFullInvoice grid with flex");
} else {
  console.log("Target to replace not found in AdminDashboard.tsx!");
}
